...
var Dice_Run = 0
var player_or_ai = [true, true, true, true] # "true" means Ai, "False" is human
var turn = 0 # turn is used for switching between players
var Dice_Face = 0 # Dice Face is used to show the top of Dice
var PlayerPosition : Array = [0, 0, 0, 0] # It stores the player's position in game
var Player_Path : Array = [] # This array holds the player sprite path
var is_running : bool = false
var one_or_two_map : bool = true # This is used to switch between boards
var Last_Won : String
var time_running : float = 0 # It counts how long the game is running
var last_updated = 0 # it helps display the time
var file_path = "res://save_score.data" # save file path
...
# This is used to save the location where the Dice needs to move
...
# This keeps the location and destination of ladders for the 1st board
...
#This keeps the location and destination of the snakes in the 1st board
...
# This keeps the location and destination of the ladders in the 2nd board
...
#This keeps the location and the destination of the snakes in the 2nd board
...
func _ready() -> void:
	...
	if one_or_two_map:
		...
		Player_Path = [$oneboard/Path/Yellow_Player, $oneboard/Path/Blue_Player, $oneboard/Path/Red_Player, $oneboard/Path/Green_Player] # Sets the Player path for the 1st board
		$twoboard.hide()  # Hides the 2nd board so it doesnt interfere
	else:
		$twoboard/Path.hide()
		$twoboard/Playing_Board.hide()
		$oneboard.hide() # Hides the 1st board so it doesnt interfere
		Player_Path = [$twoboard/Path/Yellow_Player, $twoboard/Path/Blue_Player, $twoboard/Path/Red_Player, $twoboard/Path/Green_Player] # Sets the player path for the 2nd board
	$endScreen.hide()

func change_map():
	one_or_two_map = false # Switches the board

func start_game() -> void:
	is_running = true #Sets the game as running
	$Dice_Drawed.show()
	if one_or_two_map:
		$oneboard.show()
		$oneboard/Path.show()
	...
	if not is_running: # Checks if the game is running to prevent starting
		return
	if player_or_ai[turn]: # Checks if the turn is of a player or if it is an AI
...
func _on_dice_drawed_pressed() -> void:
	if Dice_Run == 0: # Checks if It is allowed to roll yet
		return
	if $StartScreen.visible: # Checks if the game is still on startscreen
		return
	if not is_running:
		return
	$Dice_Roll.play() # Plays the Dice Roll Sound
	$Dice_Drawed.hide()
	$Dice_Animation.show()
	$Dice_Animation.play("dice_draw") # Plays the Dice rolling animation
	Dice_Run = 0 # Sets so that the Dice can't be rolled untill the value is 1
	
func ai_turn() -> void: # this function is the AI
	$Dice_Roll.play()
	$Dice_Drawed.hide()
	$Dice_Animation.show()
	$Dice_Animation.play("dice_draw")

func _on_dice_animation_finished() -> void: # This function picks a random integer from 0 to 5 and uses that as the dice number
	if not is_running:
		return
	randomize()
	...
	if PlayerPosition[turn] == 0:
		if dice_value == 1:
			...
			Player_Path[turn].position = set_offset_for_pawns(Path_Point, turn) # sets an offset so multiple pawns can be on single square
	else:
		await _move_player(turn, dice_value)
		
	await get_tree().create_timer(0.5).timeout
	if PlayerPosition[turn] >= 100: # This checks if the condition is fullfilled
		endgame()
	turn = (turn + 1) % 4 # the remainder is used as the turn mover
	Dice_Mover(turn) # This moves the Player to the Destination
	
	if player_or_ai[turn]:
		await get_tree().create_timer(1.0).timeout
		ai_turn()
	else:
		Dice_Run = 1 # Sets it so the Dice can be rolled again
	
	$Dice_Drawed/Dice_Drawed.show() # Shows the Dice rolling button so it can be pressed
...
func _move_player(player_index: int, steps: int) -> void:
	...
	if one_or_two_map:
		...
		if current in two_Ladder_Location or current in two_Snake_Location:
			...
			jump_tween.tween_property(player_node, "position", offset_final, 0.5).set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
			await jump_tween.finished
	# These start the animation of the player for the snakes and ladders
			
func Dice_Mover(player_index: int) -> void: # This function moves the Dice to every player's location
	if not is_running:
		return
